...
class WindowMenu:
    def __init__(self, screen):
        ...
        self.button1 = Button(screen, "Easy",
                              (screen.get_width() // 4, screen.get_height() // 2), 50,
                              color="white",
                              bg=(112, 128, 144), size=(300, 400))
        self.button2 = Button(screen, "Medium",
                              (screen.get_width() // 4 * 2, screen.get_height() // 2), 50,
                              color="white",
                              bg=(112, 128, 144), size=(300, 400))
        self.button3 = Button(screen, "Hard",
                              (screen.get_width() // 4 * 3, screen.get_height() // 2), 50,
                              color="white",
                              bg=(112, 128, 144), size=(300, 400))
        ...
        while running:
            for event in pygame.event.get():
                if self.button1.click(event):
                    window_print_text = WindowPrintText(self.screen, 'Easy.txt')
                    window_print_text.run()
                if self.button2.click(event):
                    window_print_text = WindowPrintText(self.screen, 'Medium.txt')
                    window_print_text.run()
                if self.button3.click(event):
                    window_print_text = WindowPrintText(self.screen, 'Hard.txt')
                    window_print_text.run()
...
class WindowPrintText:
    ...
    def __init__(self, screen, file_name):
        ...
        self.button_menu = Button(screen, "Мain Screen",
